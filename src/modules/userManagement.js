...
export const useUserManagementStore = defineStore({
    ...
    actions: {
        ...
        setUserRequest() {
            return new Promise((resolve, reject) => {
                axios.get(`http://10.164.58.65/FDTP-Portal/public/api/system-request-users/${system_id}`).then((response) => {
                    resolve(response.data)
        ...
        setAddUser(data) {
            // console.log(data)
            return new Promise((resolve, reject) => {
                axios.post('http://10.164.58.65/FDTP-Portal/public/api/role-access', data).then((response) => {
                    resolve(response.data)
        ...
        setRemoveUser() {
            return new Promise((resolve, reject) => {
                axios.get(`http://10.164.58.65/FDTP-Portal/public/api/system-load-all-users/${system_id}`).then((response) => {
        ...
        setRemoveFdtpPortalAccess(data) {
            return new Promise((resolve, reject) => {
                axios.delete(`http://10.164.58.65/FDTP-Portal/public/api/role-access/${data}`)
        ...
        setUpdatePortalRoleAccess(user_data, role_selected) {
            ...
            console.log(data)
            return new Promise((resolve, reject) => {
                axios.put(`http://10.164.58.65/FDTP-Portal/public/api/role-access/${data.id}`, data)
                    .then(function (response) {
                        resolve(response.data);
                        // console.log(response.data)
                        this.setUserManagement()
