<template>
  <div class="h-[89vh] w-full grid grid-cols-9 min-[50px]:overflow-y-scroll lg:overflow-y-hidden gap-2">
    <div class="lg:col-span-6 min-[100px]:col-span-9 h-full flex flex-col">
      ...
      <div class="flex flex-col justify-center py-4 px-16 bg-[#A10E13] rounded-full text-white">
        <span class="text-[1.5rem]">Hello, <strong>{{ first_name }}</strong></span>
        <i class="text-[1rem] opacity-90">Welcome to Hinsei & LSA Agreement List System</i>
      </div>
      ...
      <div class="grid grid-cols-2 h-[80%]">
        <div class="col-span-1 grid grid-rows-2">
          ...
          <div class="row-span-1">
            <Chart type="bar" :data="chartData2" :options="chartOptions" class="h-full" />
          </div>
          <label for="" class="ml-[5rem] text-gray-500 text-[14px]"><b><i>{{ dashboardStore.chartData.lsa_request }} Total
                LSA Request as of {{ dashboardStore.date_from }} to {{ dashboardStore.date_to }}</i></b></label>
        </div>
        <div class="col-span-1 flex flex-col justify-center items-center gap-2">
          <fieldset class="w-[40%] flex flex-col justify-center items-center gap-2 border-2">
            <legend class="ml-2">Date Filter</legend>
            <form method="post" class="w-full p-2" @submit.prevent="submitDateFilter">
              <label for="" class="flex flex-col text-[14px] w-full">
                From
                <input type="date" v-model="dashboardStore.date_from"
                  class="p-2 rounded border-2 hover:border-red-600 outline-green-600" required :max="max_date" />
              </label>
              <label for="" class="flex flex-col text-[14px] w-full">
                To
                <input type="date" v-model="dashboardStore.date_to" required
                  class="p-2 rounded border-2 hover:border-red-600 outline-green-600" :max="max_date" />
              </label>
              <button type="submit" class="bg-red-800 w-full p-2 rounded text-white hover:bg-red-700 mt-2">
                Search
              </button>
            </form>
          </fieldset>
