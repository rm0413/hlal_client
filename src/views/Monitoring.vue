<template>
  ...
  >
    ...
    <dialog
      ref="viewEditModal"
      class="p-0 rounded transform duration-300 -translate-y-5 z-10"
    >
          ...
          <div class="border-2 col-span-2 p-3 ml-2 overflow-y-auto h-[73vh]">
            ...
            >
              ...
              <template #cell(action)="data" v-if="role === 'ADMIN'">
                <div class="flex justify-center gap-1">
                  <button
                    class="h-8 w-9 rounded bg-green-700 text-white z-10"
                    v-tooltip.alignTop="'Download Attachment'"
                    @click="downloadAttachment(data.item)"
                    v-if="data.item.file_path_attachment !== null"
                  >
                  ...
                  <button
                    class="h-8 w-9 rounded bg-orange-500 text-white"
                    @click="edit_monitoring_item(data.item)"
                  >
                    <font-awesome-icon icon="pen"></font-awesome-icon>
                  </button>
                  <button
                    class="h-8 w-9 rounded bg-[#A10E13] text-white"
...
const downloadAttachment = (data) => {
  ...
  swal({
    ...
    if (response.value === true) {
      attachmentStore.downloadAttachment(datastorage).then((response) => {
