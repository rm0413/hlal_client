<template>
  <div class="h-[87vh] w-full grid grid-cols-9 min-[100px]:overflow-y-scroll lg:overflow-y-hidden gap-2">
        ...
        <c-table :items="monitoringStore.getLoadMonitoring" :fields="monitoringStore.getMonitoringFields" ref="ctable"
          ...
          <template #cell(action)="data">
            <div class="flex justify-center gap-1">
              <button @click="openModal(data.item)" data-open-modal class="h-8 w-9 rounded bg-cyan-600 text-white">
    ...
    <dialog ref="viewEditModal" class="p-0 rounded transform duration-300 -translate-y-5 z-10">
      <div class="h-[80vh] w-[90vw]">
        <div class="flex justify-between items-center h-[5vh] px-3 text-white bg-[#A10E13]">
          ...
          <button class="px-3 py-2 rounded-full hover:bg-red-600" @click="
        ...
        <div class="grid grid-cols-3 p-2 text-[13px]">
          <div class="border-2 col-span-1 p-2 h-[73vh]">
            <form class="grid grid-cols-2" action="post" @submit.prevent="submitUpdateEditItemMonitoring">
                ...
                <div>
                  <label class="flex flex-col gap-2">
                    ...
                    <textarea v-model="monitoringStore.monitoringEditItemForm.request_value
                      " style="resize: none" class="w-[33.5rem] border-2 rounded p-1 outline-green-600 h-[4.5rem]"
                      required />
                  </label>
                </div>
              </div>
            ...
            >
              ...
              <template #cell(action)="data" v-else>
                <div class="flex justify-center gap-1">
                  ...
                  <button class="h-8 w-9 rounded bg-gray-400 text-white" @click="edit_monitoring_item(data.item)"
                    disabled>
                    <font-awesome-icon icon="pen"></font-awesome-icon>
                  </button>
                  <button class="h-8 w-9 rounded bg-gray-400 text-white" @click="delete_monitoring_item(data.item)"
                    disabled>
                    <font-awesome-icon icon="trash" />
                  </button>
                </div>
      ...
      monitoringStore
        ...
        .then((response) => {
          if (response.status === "success") {
            ...
            toast.add({ severity: 'error', summary: 'Warning', detail: response.message, life: 2000, });
          }
        });
    } else {
...
const downloadAttachment = (data) => {
  ...
  swal({
    ...
    if (response.value === true) {
      window.location.href = `http://10.164.58.62/hinsei/server/public/download-attachment?file_path_attachment=${path}`;
      viewEditModal.value.showModal();
    } else {
      viewEditModal.value.showModal();
      toast.add({ severity: 'error', summary: 'Warning', detail: 'Cancelled.', life: 2000 });
