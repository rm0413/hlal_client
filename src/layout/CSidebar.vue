  ...
  <div>
    ...
    <div
      ...
      <div @click="openAside()" ref="asideCog"
        ...
        <font-awesome-icon :class="`text-${themeStore.theme[themeStore.theme_select]._navbar_icon} h-6 w-6`"
          icon="gear"></font-awesome-icon>
      ...
      <div ref="maxAside"
        ...
        <div class="text-xl text-left flex flex-col p-5">
          <div class="w-full flex items-center justify-center">
            <img v-if="userDetails.photo !== undefined" :src="userDetails.photo" alt="" class="rounded-full w-[7rem]">
      ...
      <div ref="maxSidebar" :class="`max hidden text-white mt-20 flex-col space-y-2 w-full h-[calc(100vh)]`">
        <div v-for="(navs, n) in links" :key="n">
          <router-link v-if="!navs.sub_module" :to="{ name: navs.name }"
            ...
            <font-awesome-icon v-if="navs.fa"
          ...
          <div v-for="(sub, s) in navs.sub_module" :key="s">
            <router-link :to="{ name: sub.name }">
              <div ref="sub_module"
                :class="`hover:ml-4 w-full shadow-lg text-${themeStore.theme[themeStore.theme_select].sidebar_text} hover:text-rose-900 text-[12px] dark:hover:text-blue-500 bg-${themeStore.theme[themeStore.theme_select].sidebar_route_bg} border-red-700 p-2 pl-12 rounded transform ease-in-out duration-300 flex flex-row items-center space-x-3 shadow-lg cursor-pointer gap-2`">
    ...
    <div ref="content" :class="`content ml-12 transform ease-in-out duration-500 pt-20 px-2 md:px-5 pb-4 h-screen`">
      <slot name="page"></slot>
    </div>
    <div
...
const expand_submodule = () => {
  sub_module.value.forEach((v, i) => {
    if (v.classList.contains("opacity-0")) {
      // v.classList.remove('hidden')
      v.classList.remove("-translate-x-[15rem]");
      v.classList.remove("opacity-0");
      v.classList.remove("absolute");
    } else {
      v.classList.add("opacity-0");
      // v.classList.add('hidden')
      v.classList.add("-translate-x-[15rem]");
      setTimeout(() => {
