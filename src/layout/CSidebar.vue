<template>
  <div>
    ...
    ></div>
    <div
      :class="`select-none fixed w-full z-30 flex shadow-lg dark:bg-[#0F172A] p-2 items-center justify-center h-16 px-10 bg-${
        themeStore.theme[themeStore.theme_select]._navbar
      }`"
    >
      <div
        ref="logo"
      ...
      <div
        ...
        :class="`bg-gradient-to-r duration-500 bg-${
          themeStore.theme[themeStore.theme_select]._navbar
      ...
      >
        ...
        <div
          ref="settings"
          :class="`opacity-0 duration-[600ms] font-bold md:block text-sm md:text-md text-${
            themeStore.theme[themeStore.theme_select].sidebar_text
      ...
      >
        <font-awesome-icon
          :class="`text-${
            themeStore.theme[themeStore.theme_select]._navbar_icon
          } h-6 w-6`"
          icon="gear"
        ></font-awesome-icon>
    ...
    <aside
      ref="caside"
      :class="`select-none inset-y-0 right-0 w-[19rem] translate-x-96 fixed transition ease-in-out duration-500 flex z-10 h-[calc(100vh)] shadow-2xl bg-${
        themeStore.theme[themeStore.theme_select].aside
      }`"
    >
      <!-- MAX ASIDE -->
      <div
    ...
    <aside
      ref="sidebar"
      :class="`select-none w-60 -translate-x-48 fixed transition transform ease-in-out duration-1000 z-30 flex h-screen bg-${
        themeStore.theme[themeStore.theme_select].sidebar
      } shadow-xl`"
    >
      <!-- open sidebar button -->
      <div
        ref="maxToolbar"
        :class="`max-toolbar translate-x-24 scale-x-0 w-full -right-6 transition transform ease-in duration-300 flex items-center justify-between border-2 border-gray-200 dark:border-[#0F172A] bg-${
          themeStore.theme[themeStore.theme_select].container
        } shadow-lg absolute top-2 rounded-full h-12`"
      >
        <div class="flex pl-4 items-center space-x-2">
          <div>
            <div
              @click="themeStore.darkMode(), openNav()"
              v-if="themeStore.theme_select === 0"
      ...
      >
        <div v-for="(navs, n) in links" :key="n">
          <router-link
            ...
            :to="{ name: navs.name }"
            :class="`hover:ml-4 w-full shadow-lg text-${
              themeStore.theme[themeStore.theme_select].sidebar_text
            } hover:text-rose-900 dark:hover:text-blue-500 bg-${
          ...
          >
            ...
            <font-awesome-icon
              v-if="navs.fa"
              :class="`h-3 w-3 text-${
                themeStore.theme[themeStore.theme_select].sidebar_icon
              }`"
              :icon="navs.fa"
            ></font-awesome-icon>
          ...
          <div
            :class="`hover:ml-4  w-full shadow-lg text-${
              themeStore.theme[themeStore.theme_select].sidebar_text
            } hover:text-rose-900 dark:hover:text-blue-500 border-red-700 p-2 pl-8 flex-row items-center space-x-3 transform ease-in-out duration-300 flex shadow-lg cursor-pointer bg-${
              themeStore.theme[themeStore.theme_select].sidebar_route_bg
            } ${!openSubModule ? ' rounded text-[#A10E13]' : ' rounded'}`"
            @click="(openSubModule = !openSubModule), expand_submodule(`submodule(${n})`)"
            v-else
          >
            <i v-show="navs.class" :style="navs.style" :class="navs.class"></i>
            <i v-show="navs.svg" v-html="navs.svg"></i>
          ...
          <div v-for="(sub, s) in navs.sub_module" :key="s">
            <router-link :to="{ name: sub.name }">
              <div
                ref="sub_module"
                :class="`submodule(${n}) hover:ml-4 w-full shadow-lg text-${
                  themeStore.theme[themeStore.theme_select].sidebar_text
                } hover:text-rose-900 text-[12px] dark:hover:text-blue-500 bg-${
                  themeStore.theme[themeStore.theme_select].sidebar_route_bg
      ...
      >
        <div v-for="(navs, n) in links" :key="n">
          ...
          <div
            :class="` hover:ml-4 justify-end pr-5 hover:text-red-900 dark:hover:text-red-200 duration-500  w-full p-3 bg-${
              themeStore.theme[themeStore.theme_select].sidebar_route_bg
            }  ${
    ...
    <div
      :class="`select-none absolute inset-x-0 bottom-0 w-screen duration-500 bg-${
        themeStore.theme[themeStore.theme_select].footer
